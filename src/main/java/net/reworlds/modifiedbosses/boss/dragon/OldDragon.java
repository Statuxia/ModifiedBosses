...
public class OldDragon {
    ...
    public static boolean findDragon(int numTry) {
        ...
        if (OldDragon.dragon != null && !OldDragon.dragon.isDead()) {
...
//                oldNearPlayer.addAll(nearPlayers);
                ...
                attackedBy.forEach((player, damage) -> {
                    if (!nearPlayers.contains(player) && OldDragon.getDragon().getHealth() > 100) {
                        toRemove.add(player);
                    }
                });
                ...
                lastAbility = OldAbilities.activate();
